...
import { useState, useEffect } from 'react';
import { Card, CardContent, Typography, Box, CircularProgress, useTheme, CardActions, Button, Divider, LinearProgress } from '@mui/material';
import dayjs from 'dayjs';
import { VirtualSip } from '@/models/VirtualPortfolio';
import Link from 'next/link';
// FIX: Import the safe formatCurrency function from your utils file
import { formatCurrency } from '@/lib/utils';
...
export default function VirtualSipCard({ sip, onSipDeleted, onSipUpdated }: { sip: VirtualSip, onSipDeleted: (id: string) => void, onSipUpdated: () => void }) {
    ...
    // The local, unsafe version of formatCurrency has been removed.

    useEffect(() => {
        async function calculatePerformance() {
            try {
                const res = await fetch(`/api/scheme/${sip.schemeCode}/sip`, {
                    method: 'POST',
                ...
                if (!res.ok) throw new Error("Could not calculate performance");
    ...
    const handleDelete = async () => {
        if (!window.confirm("Are you sure you want to delete this virtual SIP?")) return;
        
        try {
            await fetch(`/api/portfolio/${sip._id}`, { method: 'DELETE' });
            onSipDeleted(sip._id.toString());
        } catch (error) {
            console.error("Failed to delete SIP");
        }
    };

    const gain = performance ? performance.currentValue - performance.totalInvested : 0;
    const gainColor = gain >= 0 ? theme.palette.success.main : theme.palette.error.main;
    // FIX: Safely calculate return percentage
    const returnPercentage = performance?.totalInvested > 0 ? (gain / performance.totalInvested) * 100 : 0;
    const progress = sip.durationMonths > 0 ? (sip.completedInstallments / sip.durationMonths) * 100 : 0;

    return (
        <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <CardContent sx={{ flexGrow: 1 }}>
                <Typography variant="h6" fontWeight={600} noWrap>{sip.schemeName}</Typography>
                <Typography variant="body2" color="text.secondary">
                    {/* This now uses the safe, imported function */}
                    SIP: {formatCurrency(sip.sipAmount)} / month since {dayjs(sip.startDate).format('MMM YYYY')}
                </Typography>
                    ...
                    <Box>
                        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
                            <Box>
                                ...
                                <Typography fontWeight={600}>{formatCurrency(performance.totalInvested)}</Typography>
                            </Box>
                            <Box>
                                <Typography variant="caption" color="text.secondary">Current Value</Typography>
                                <Typography fontWeight={600}>{formatCurrency(performance.currentValue)}</Typography>
                            </Box>
                             <Box>
                                <Typography variant="caption" color="text.secondary">Gain/Loss</Typography>
                                <Typography fontWeight={600} sx={{ color: gainColor }}>{formatCurrency(gain)}</Typography>
                            </Box>
                            <Box>
                                <Typography variant="caption" color="text.secondary">Abs. Return</Typography>
                                <Typography fontWeight={600} sx={{ color: gainColor }}>{returnPercentage.toFixed(2)}%</Typography>
                            </Box>
                        </Box>
                        {sip.durationMonths > 0 && (
                            <Box sx={{ mt: 2 }}>
                                <Typography variant="caption" color="text.secondary">
            ...
            <CardActions>
                ...
                <Button size="small" disabled>{sip.status === 'active' ? 'Pause' : 'Resume'}</Button>
            </CardActions>
        </Card>
    );
}