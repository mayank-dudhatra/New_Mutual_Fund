// src/app/api/portfolio/[id]/pause/route.ts
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        if (result.modifiedCount === 0) {
            return NextResponse.json({ error: "SIP not found or not active" }, { status: 404 });
        }
        return NextResponse.json({ success: true, message: "SIP has been paused." });
    } catch (error) {
